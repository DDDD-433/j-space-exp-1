# Portions derived from anthropics/jacobian-lens (jlens/_logging.py)
# Copyright 2026 Anthropic PBC
# Copyright 2026 The OpenJSpace Authors
# SPDX-License-Identifier: Apache-2.0
"""Opt-in stderr logging for the ``openjspace`` logger."""
...
class _DeltaFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        now = time.monotonic()
        record.elapsed = f"{human_duration(now - self._t0):>7}"
...
def configure_logging(level: int | str = logging.INFO) -> None:
    ...
    the ``openjspace`` logger. Idempotent; library callers that configure
    :mod:`logging` themselves should not call this."""
    package_logger = logging.getLogger("openjspace")
